...
static void adjustCapacity(Table* table, int capacity) {
  ...
  table->count = 0;
  // now we need to copy the existing buckets over to our new entries
  for (int i = 0; i < table->capacity; i++) {
    Entry* entry = &table->entries[i];
    if (entry->key == NULL) continue;

    // where does it go in the new bucket?
...
bool tableSet(Table* table, ObjString* key, Value value) {
  // allocate / grow array of entries
...
bool tableDelete(Table* table, ObjString* key) {
  ...
  // tombstone the value so we don't break our chain when linear probing
  // use a null key and a bool value so it's different than an empty bucket
  entry->key = NULL;
...
void tableRemoveWhite(Table* table) {
  for (int i = 0; i < table->capacity; i++) {
    Entry* entry = &table->entries[i];
    // remove string from the table if not marked
    // basically deleting from the table any string that isn't referenced
